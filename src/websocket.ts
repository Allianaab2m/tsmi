...
export type Channels = {
  main: {
    ...
    events: {
      ...
      mention: (payload: INote) => void;
      reply: (payload: INote) => void;
      renote: (payload: INote) => void;
      follow: (payload: IUserLite) => void; // 自分が他人をフォローしたとき
      followed: (payload: IUserLite) => void; // 他人が自分をフォローしたとき
      unfollow: (payload: IUserLite) => void; // 自分が他人をフォロー解除したとき
      readAllNotifications: () => void;
      readAllUnreadMentions: () => void;
      unreadSpecifiedNote: (payload: INote["id"]) => void;
...
export abstract class Connection<Channel extends AnyOf<Channels> = any>
  extends TypedEventEmitter<Channel["events"]> {
  public channel: string;
  protected stream: Stream;
...
class SharedConnection<Channel extends AnyOf<Channels> = any>
...
class NonSharedConnection<Channel extends AnyOf<Channels> = any>
  extends Connection<Channel> {
  public id: string;
...
export const webSocketHandler = (
  ...
  const uuid = uuidV4();
  console.log("Connection ID:", uuid);
  channels.forEach((c) => {
    ws.send(
      JSON.stringify({
        type: "connect",
  ...
  ws.addEventListener("close", () => {
    ...
    channels.forEach((c) => {
      console.log("[WebSocket] Disconnecting...", `${uuid}-${c}`);
    ...
    webSocketHandler(channels, ws, client);
  });
}; */
